...
def clean_and_constrain_data(df: pd.DataFrame) -> pd.DataFrame:
    """
    Clean and constrain the values in the dataset to realistic ranges and decimal places.
    
    Args:
        df: Input DataFrame with simulated housing data
    
    Returns:
        pd.DataFrame: Cleaned and constrained housing data
    """
    ...
    rounding_dict = {
        'price': 0,                    # Whole numbers for price
        'num_rooms': 0,                # Whole numbers for rooms
        'num_bathrooms': 0,            # Whole numbers for bathrooms
        'size_m2': 0,                  # Whole numbers for size
        'lot_size_m2': 0,              # Whole numbers for lot size
        'house_age': 0,                # Whole numbers for age
        'altitude': 0,                 # Whole numbers for altitude
        'crime_rate': 2,               # Two decimals for rates
        'school_quality_index': 1,     # One decimal for indices
        'green_space_proportion': 3,   # Three decimals for proportions
        'noise_pollution': 1,          # One decimal for noise
        'sunlight_hours': 1,           # One decimal for hours
    }
    
    # Apply rounding
    for column, decimals in rounding_dict.items():
        if column in df.columns:
            df[column] = df[column].round(decimals)
    
    # Ensure integer columns are integers
    integer_columns = ['num_rooms', 'num_bathrooms', 'house_age']
    ...
    constraints = {
        ...
        'house_age': (0, 100),
        'crime_rate': (0, 100),
        'school_quality_index': (0, 10),
        'green_space_proportion': (0, 1),
        'noise_pollution': (30, 90),
        'sunlight_hours': (3, 9),
        'altitude': (3000, 4200)
    }
...
if __name__ == "__main__":
    ...
    # Clean and constrain the data
    housing_data = clean_and_constrain_data(housing_data)
